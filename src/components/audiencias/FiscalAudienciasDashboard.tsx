...
export function FiscalAudienciasDashboard() {
  ...
  useEffect(() => {
    if (profile?.id) {
      fetchAudienciasFiscal();
    }
  }, [profile?.id]);

  const fetchAudienciasFiscal = async () => {
    try {
      ...
      console.error('Erro ao buscar audiências do fiscal:', error);
    } finally {
      setIsLoading(false);
    }
  };
