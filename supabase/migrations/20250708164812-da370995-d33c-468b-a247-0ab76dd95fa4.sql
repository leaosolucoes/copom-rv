...
-- Inserir endpoints padrão da API
INSERT INTO public.api_endpoints (path, method, description, required_scopes, rate_limit_per_hour) VALUES
-- Token Management
('/api/auth/generate-token', 'POST', 'Gerar novo token de API', '{"token:create"}', 100),
('/api/auth/validate-token', 'POST', 'Validar token de API', '{"token:validate"}', 2000),
('/api/auth/refresh-token', 'POST', 'Renovar token de API', '{"token:refresh"}', 100),

-- Users Management
('/api/users', 'GET', 'Listar usuários', '{"users:read"}', 1000),
('/api/users/{id}', 'GET', 'Obter usuário específico', '{"users:read"}', 1000),
('/api/users', 'POST', 'Criar usuário', '{"users:write"}', 100),
('/api/users/{id}', 'PUT', 'Atualizar usuário', '{"users:write"}', 100),
('/api/users/{id}', 'DELETE', 'Deletar usuário', '{"users:delete"}', 50),

-- Complaints Management
('/api/complaints', 'GET', 'Listar denúncias', '{"complaints:read"}', 1000),
('/api/complaints/{id}', 'GET', 'Obter denúncia específica', '{"complaints:read"}', 1000),
('/api/complaints', 'POST', 'Criar denúncia', '{"complaints:write"}', 500),
('/api/complaints/{id}', 'PUT', 'Atualizar denúncia', '{"complaints:write"}', 200),
('/api/complaints/{id}/status', 'PATCH', 'Atualizar status da denúncia', '{"complaints:write"}', 200),
('/api/complaints/{id}', 'DELETE', 'Deletar denúncia', '{"complaints:delete"}', 50),
('/api/complaints/{id}/media', 'GET', 'Obter mídia da denúncia', '{"complaints:read"}', 1000),

-- System Settings
('/api/settings', 'GET', 'Listar configurações', '{"settings:read"}', 1000),
('/api/settings/{key}', 'GET', 'Obter configuração específica', '{"settings:read"}', 1000),
('/api/settings/{key}', 'PUT', 'Atualizar configuração', '{"settings:write"}', 100),

-- CNPJ & External Data
('/api/cnpj/{cnpj}', 'GET', 'Consultar CNPJ', '{"cnpj:read"}', 500),
('/api/cnpj/{cnpj}/pdf', 'GET', 'Gerar PDF da consulta CNPJ', '{"cnpj:read"}', 200),

-- File Management
('/api/upload/media', 'POST', 'Upload de mídia', '{"files:write"}', 200),
('/api/upload/{id}', 'GET', 'Download de arquivo', '{"files:read"}', 1000),
('/api/upload/{id}', 'DELETE', 'Deletar arquivo', '{"files:delete"}', 100);
