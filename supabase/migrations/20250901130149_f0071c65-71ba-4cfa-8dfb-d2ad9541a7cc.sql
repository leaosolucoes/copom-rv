-- Criar trigger para envio automático de WhatsApp para imprevistos na viatura
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    IF whatsapp_config.auto_send_enabled AND 
      ...
      message_text := REPLACE(message_text, '{data_imprevisto}', COALESCE(NEW.created_at::text, ''));
...
-- Criar o trigger na tabela escala_imprevistos
DROP TRIGGER IF EXISTS trigger_whatsapp_imprevisto_notification ON public.escala_imprevistos;
CREATE TRIGGER trigger_whatsapp_imprevisto_notification
  AFTER INSERT ON public.escala_imprevistos
  FOR EACH ROW
  EXECUTE FUNCTION public.send_whatsapp_imprevisto_notification();